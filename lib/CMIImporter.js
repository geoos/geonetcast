...
class CMIImporter {
    ...
    async getSourceFiles(afterTime) {
        try {
            let files = [];
    ...
    async importFile(f) {
        try {
            ...
            let fileName = "gnc-cmi_[" + varName + "]" + fileTime.format("YYYY-MM-DD_HH-mm") + ".grb2";
            let warpped = "/home/data/working/" + fileName;
            let warpCmd = `gdalwarp -multi -s_srs "+proj=geos +lon_0=-75 +h=35786023 +sweep=x +a=6378137 +b=6356752.31414" NETCDF:${unpacked}:CMI -t_srs "+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0" -of GRIB -overwrite ${warpped}`;
    ...
    async daemon() {
        ...
        this.running = true;
        try {
            log.debug("Importer daemon running");
            let imported = false;
            do {
                let state = this.getState();
                if (!state.lastTime) state.lastTime = 0;
                let files = await this.getSourceFiles(state.lastTime);
                if (files.length) {
                    imported = true;
                    let f = files[0];
                    state.lastTime = f.time;
                    try {
            ...
            log.debug("Importer daemon finished");
        } catch(error) {
            console.error(error);
